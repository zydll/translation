...
def show(request, file_path):
    trans_texts = {}
    for t in Py3Sentence.objects.filter(file_path=file_path).values('sid', 'trans_text'):
        trans_texts[t['sid']] = mark_safe(t['trans_text'])
    return render_to_response('python_3/%s' % file_path, {'s': trans_texts})
    
def origion(request, file_path):
    origin_texts = Py3Sentence.objects.filter(file_path=file_path).values('sid', 'orig_text')
...
def translation(request, file_path, sid):
    if request.method == 'GET':
        translations = Py3Translation.objects.filter(file_path=file_path, sid=sid).values('id', 'trans_text', 'plus', 'minus')
        ...
        snt = Py3Sentence.objects.get(file_path=file_path, sid=sid)
        score, factor = _compare_difference(snt.trans_text, translation)
        if  score < factor * _threshold:
        ...
        t = Py3Translation(file_path=file_path, sid=sid, trans_text=translation, plus=plus, minus=minus)
        t.save()
        if (plus - minus > pre_vote):
            s = Py3Sentence.objects.filter(file_path=file_path, sid=sid)
            s.update(trans_text=translation)
        return HttpResponse(plus - minus - pre_vote)
    
def plus_vote(request, file_path, tid):
    t = Py3Translation.objects.get(id=tid, file_path=file_path)
    Py3Translation.objects.filter(id=tid, file_path=file_path).update(plus = t.plus + 1)
    return HttpResponse(t.plus + 1)        
    
def minus_vote(request, file_path, tid):
    t = Py3Translation.objects.get(id=tid, file_path=file_path)
    Py3Translation.objects.filter(id=tid, file_path=file_path).update(minus = t.minus + 1)
