...
function showOriginTexts() {
  if(window.isOriginTextsGenerated) {
    $(".or").show();
  } else {
    $("div.body p,div.body li").each(function() {
      if($(this).find("p").length > 0 || $(this).find("li").length > 0){

      } else {
        node_or = $(this).clone();
        node_or.addClass("or");
        node_or.children("font").each(function() {
          id = $(this).attr("id");
          $(this).attr("id", "or" + id);
          $(this).html(window.origion_texts[id]);
        });
        $(this).append(node_or);
      }
    })
    window.isOriginTextsGenerated = true;
  }
}

function hideOriginTexts() {
  $(".or").hide();
}

function getStatsTab() {
  return $('<div class="MSTCStatsTab" style="margin-top: 2px;"><div class="MSTCVoteCount MSTCEdit" title="编辑此翻译" style="margin-top: 1px; text-align: center; border-top-left-radius: 2px; border-bottom-left-radius: 2px; background-color: #3D98CD;"><span><font style="display: inline-block; margin: 4px 1px 0px 3px;"></font><img src="/python_3/_static/ctfedit.gif" style="margin: 7px 3px 0px 1px;"></span></div><div class="MSTCVoteCount MSTCPlus" title="建议将此翻译作为更好的翻译" style="margin-top: 1px; text-align: center; border-top-left-radius: 2px; border-bottom-left-radius: 2px; background-color: #3D98CD;"><span><font style="display: inline-block; margin: 4px 1px 0px 3px;"></font><img src="/python_3/_static/ctfvotes.gif" style="margin: 7px 3px 0px 1px;"></span></div><div class="MSTCVoteCount MSTCMinus" title="觉得此翻译不够好" style="margin-top: 1px; text-align: center; border-top-left-radius: 2px; border-bottom-left-radius: 2px; background-color: rgb(191, 42, 45);"><span style="background-color: rgb(191, 42, 45);"><font style="display: inline-block; margin: 4px 1px 0px 3px;">0</font><img src="/python_3/_static/ctfreport.gif" style="margin: 7px 3px 0px 1px;"></span></div></div>');
}
...
function showHtmlEdit() {
  ...
  $('div.MSTCTransPanelHover #MSTCTransHtmlEdit').children('iframe').contents().find("head").append($('<link rel="stylesheet" type="text/css" href="/python_3/_static/basic.css">'))
...
$(document).ready(function(){
  ...
  getOriginTexts();
  window.boolShowEdit = false;
  window.isOriginTextsGenerated = false;
  ...
  $(document).delegate('font', 'mouseover', function() {
    if($(this).parent().hasClass("or")) {
      return;
    }
    if(!window.boolShowEdit) {
      window.hover_id = $(this).attr("id");
      window.sbg_color = $('#' + window.hover_id).css("background-color");
      window.sft_color = $('#' + window.hover_id).css("color");
      $('#' + window.hover_id).css("background-color", "#F0F0A0");
      $('#' + window.hover_id).css("color", "#0F0F55");
      $('#or' + window.hover_id).css("background-color", "#F0F0A0");
      $('#or' + window.hover_id).css("color", "#0F0F55");
    }
    window.timeout_st = setTimeout(checkShowTranslator , 1010);
  });

  $(document).delegate('font', 'mouseout', function() {
    if($(this).parent().hasClass("or")) {
      return;
    }
    clearTimeout(window.timeout_st);
    if(!window.boolShowEdit) {
      $(this).css("background-color", window.sbg_color);
      $(this).css("color", window.sft_color);
      $('#or' + window.hover_id).css("background-color",  window.sbg_color);
      $('#or' + window.hover_id).css("color", window.sft_color);
      $("#MicrosoftTranslator").stop(true,true).delay(1000).hide(0);
    }
  });
  ...
  $("#MicrosoftTranslator").hover(function() {
    if(!window.boolShowEdit) {
      $('#' + window.sid).css("background-color", "#F0F0A0");
      $('#' + window.sid).css("color", "#0F0F55");
      window.clickPage = false;
    ...
    if(!window.boolShowEdit && !window.clickPage) {
      ...
      $('#' + window.sid).css("color", window.sft_color);
